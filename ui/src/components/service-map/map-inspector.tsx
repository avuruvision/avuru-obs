...
export function MapInspector({ service, selected, edges, health, carbon, logs, onSelect, onClear, onFocus }: {
  ...
  ) : <p className="text-xs text-base-content/65">None observed in this view.</p>;
  return (
    <aside aria-label="Service inspector" data-testid="map-inspector" className="flex min-w-0 flex-col gap-5 border-l border-neutral bg-base-200 p-5">
      <div>
        <div className="flex items-center justify-between gap-2">
          <p className="text-[10px] font-medium uppercase tracking-[0.16em] text-base-content/65">{inferred ? "Inferred dependency" : peer ? "Undetected peer" : "Selected service"}</p>
        ...
        <p className="mt-1 break-all text-xs text-base-content/65">{service.namespace || "Namespace not reported"}</p>
      ...
      <div className="border-y border-neutral py-4">
        <p className="text-[10px] uppercase tracking-wider text-base-content/65">Service p95 latency</p>
        ...
        {carbon && observed && <div className="mt-4 border-t border-neutral pt-3 text-xs">
          ...
          <p className="mt-2 text-base-content/65">The map does not report measured/estimated provenance. Check the Green view before interpreting this value.</p>
