-- Metrics schema v1 (kubeletstats infra metrics + any OTLP metrics).
-- Column contract frozen VERBATIM from clickhouseexporter 0.154.0 against
-- ClickHouse 26.3 via the contract-freeze procedure (create_schema:true run +
-- SHOW CREATE TABLE, see gateway/schemas/README.md). All five metric-type
-- tables are created even though kubeletstats only emits gauges/sums: with
-- create_schema:false a missing table means silently dropped inserts the day
-- any OTLP-metrics app shows up.
