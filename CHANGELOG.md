...
- **Turning on network flows stopped the sensor instead of enriching it.**
  ...
  them. Per-edge network health has therefore never worked on a real cluster
  since it shipped in v0.2.0 — if you enabled it, it is worth re-checking the
  sensor pod. Rendered configs are now parsed in the chart's own tests, so a
  document the sensor cannot load fails the build.
