...
## v0.12 — the spend you can act on — SHIPPED (v0.12.0)
...
| **An agent can read the estate** | Not planned for this line, and shipped in it. Everything above teaches the product about agents; nothing let an agent know anything about the estate. An **MCP module** serves six read-only tools over the traces, logs, error issues and health already stored — no new collection, no schema change, no container, one handler on the hub authenticated with the personal tokens that exist since v0.5. **Off by default**, because what an agent reads leaves the cluster for whichever model provider the operator chose, which is a decision to hand over rather than to make for them — [AEP](design/2026-09-01-mcp-server.md) |
...
| **A mesh you can actually read** | The Mesh screen answered two questions and, on an ambient cluster, rendered two rows. Proxies gain a **role** (control plane, gateway, waypoint, ztunnel, sidecar) and a namespace from labels storage was already keeping and nobody read; two columns that said bytes and rendered call counts are renamed, and real bytes and link health take columns of their own; a proxy opens on what it *carries* — the collapsed dependencies recovered through it, read from the other end. Then a second, separately-granted module reads the cluster's own Istio and Gateway API objects, so a namespace that is enrolled and **silent** finally has a row, and six checks name the misconfigurations that emit no telemetry at all — [AEP](design/2026-09-06-mesh-by-role.md), [config](design/2026-09-07-mesh-configuration.md) |
